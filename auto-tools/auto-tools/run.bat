...
start cmd /c "cd chocolatey && "get-packages.sh""
...
:jobs_running
wmic process where (parentprocessid="%PID%" and name="cmd.exe") | findstr /l "No Instance(s) Available."
if %ERRORLEVEL%==0 timeout 1 /nobreak && goto jobs_running
